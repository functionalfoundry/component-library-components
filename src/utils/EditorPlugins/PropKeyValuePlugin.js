...
class JSXPropName extends React.Component {
  ...
  render() {
    const { theme, children, mark } = this.props
...
const JSXPropValue = ({ theme, children, mark }: JSXDecoratorPropsT) => {
        ...
        if (index < node.value.expression.start || index >= node.value.expression.end) {
          ...
          let mark = Slate.Mark.create({
            ...
            data: {
              ...
              propKeyValue: getPropKeyValueByName(options.propKeyValues, node.name.name),
      ...
      nodes: {
        code: {
          decorate: combineDecorators(
            [decorateJSXComponentNames, decorateJSXPropNames, decorateJSXPropValues],
            options
          ),
        },
      },
